...
(\S\ref{sec:ablation}).} Projecting the same frozen CV-LR direction out
at round~4 produces $-0.9\%$ ($p{=}0.57$). The deterministic-LR frozen
direction also nulls ($+2.4\%$). Only difference-of-means retains
measurable necessity ($-7.2\%$ frozen; $-36.5\%$ fresh, the largest
effect in the experiment). \textbf{Additive steering alone does not
license necessity claims.}
\end{enumerate}
...
\paragraph{Ablation measured at a single checkpoint.} We ran directional
ablation at round~4 only. Whether the dissociation is present at
earlier checkpoints (including round~2, which is more responsive to
additive steering) is untested here.
